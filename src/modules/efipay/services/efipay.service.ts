...
export class EfiPayService {
  ...
  constructor(
    ...
    const efiCredentials =
      configLoaderHelper().efipay.mode === "sandbox"
        ? configLoaderHelper().efipay.sandbox
        : configLoaderHelper().efipay.production;
    this.efipayInstance = new EfiPay({
      ...
      certificate: `src/certs/efipay/${efiCredentials.certificateName}.pem`,
    });
  }
  //up
